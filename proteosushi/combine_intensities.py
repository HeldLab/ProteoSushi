...
def __localization_rollup(search_engine: str, search_engine_filepath: str, use_target_list: bool, 
    ...
    if search_engine == "maxquant":
        sequence_index, modified_index, localization_indices, mod_dict, intensity_start, \
    ...
    total_seqs = 0
    over_threshold = 0
    unmatched_sequences = list()

    for row in tsv_reader:
        # If a maxquant file is used and the false disc rate of this peptide is not below the threshold, skip it
        if not fdr_threshold is None and float(row[false_disc]) > fdr_threshold:
            over_threshold += 1
            continue
        total_seqs += 1
        raw_seq = row[sequence_index]
        pep_mod_seq = row[modified_index].strip('_')
        #loc_seqs = [row[i] if header[i].split(" Probabilities")[0] in user_PTMs else "" for i in localization_indices]#
        ...
        new_pep_mod_seq, new_pep_seq, missed_cleave_fix = clean_pep_seq(cleave_rules[protease], 
                                                                        pep_mod_seq, 
                                                                        new_user_PTMs, 
                                                                        raw_seq,
                                                                        True)
        #new_loc_seqs, new_pep_seq, missed_cleave_fix = clean_localization_pep_seq(cleave_rules[protease], 
        #                                                                loc_seqs, 
        #                                                                new_user_PTMs, 
        #                                                                raw_seq)#
        ...
        if genes_positions and len(genes_positions) == 1:
            ...
            if not new_pep_mod_seq in mod_dict:
                print("\033[91m {}\033[00m".format(f"{raw_seq} does not have the PTM(s) selected!"))  # Add in below loc threshold as well
                missing_PTM += 1
                continue
...
def rollup(search_engine: str, search_engine_filepath: str, use_target_list: bool, 
    ...
    if search_engine == "generic":
        ...
        if not localization_threshold is None:
            return __localization_rollup(search_engine, search_engine_filepath, use_target_list, 
                                         target_list_filepath, max_missed_cleavages, protease, 
                                         fdr_threshold, use_quant, user_PTMs, 
                                         proteome_fasta_filepath, intensity_method, add_annotation, 
                                         species_id, output_filename, localization_threshold)
        sequence_index, modified_sequence_index, mod_dict, intensity_start, data_filename, \
